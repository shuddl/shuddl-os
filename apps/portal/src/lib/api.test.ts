...
describe("portal api client (REQ-085)", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  afterEach(() => {
  ...
  it("targets the base URL from VITE_API_BASE", async () => {
    ...
    await get("/pub/status/SHP-1");

    expect(apiBase()).toBe("https://api.stub.example");
    expect(fetchMock.mock.calls[0]?.[0]).toBe("https://api.stub.example/pub/status/SHP-1");
  });
  ...
  it("a POST attaches a FRESH Idempotency-Key and the bearer token, and JSON-encodes the body", async () => {
    ...
    await post("/v1/bookings", { origin: "A" });
    await post("/v1/bookings", { origin: "B" });
    ...
    expect(fetchMock.mock.calls[0]?.[1]?.body).toBe(JSON.stringify({ origin: "A" }));
  });

  it("a /pub/* GET works with NO token (no Authorization, no Idempotency-Key)", async () => {
    clear();
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ status: "IN TRANSIT" }));
    vi.stubGlobal("fetch", fetchMock);

    const out = await get<{ status: string }>("/pub/status/SHP-1");

    expect(out.status).toBe("IN TRANSIT");
  ...
  it("a 401 surfaces a distinguishable re-auth signal carrying the envelope code + req_id", async () => {
    ...
    const err = await get("/v1/shipments").catch((e: unknown) => e);

    expect(err).toBeInstanceOf(ApiError);
    const apiErr = err as ApiError;
    expect(apiErr.status).toBe(401);
    expect(apiErr.code).toBe("UNAUTHORIZED");
    expect(apiErr.isAuthError).toBe(true); // the view drops the session + re-prompts for a magic link
  ...
  it("a non-401 error still throws a typed ApiError with the code, isAuthError false", async () => {
    ...
    const err = await post("/v1/bookings", {}).catch((e: unknown) => e);
