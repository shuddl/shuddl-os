...
- Gate refusals cross a Durable Object → Workers RPC hop that preserves ONLY `Error.name` + `Error.message`. So the machine-readable detail is encoded INTO the message as `CODE:{json}`: `GATE_BLOCKED:{"required_evidence":[...]}` (`gates/invoice-gate.ts:12`) and `VALIDATION_FAILED:{"reason":...}` (`gates/transition-gates.ts:60`). Both codes are in `contracts/src/errors.ts`. Do not "clean up" these strings into structured throws — the structure would not survive the hop.
