...
`tools/checks/invariants.ts:204` — `FORBIDDEN_REPLACE` (the TS-source scanner) is a hand-written copy:
...
## Common Mistakes
- **Copying the pattern instead of the fragment.** The `invariants.ts:204` copy dropped `SCHEMA` and downgraded `DELIM` to `\s+`. Extract `replaceTarget()`; both surfaces import it.
