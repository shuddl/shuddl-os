...
| Every gated append traverses ONE writer — the sequencer DO — so no route can reach `INSERT INTO events` around a gate (CLAUDE.md rule 3 / REQ-030 / I3) | `pnpm check:chokepoint` | M | Synthesized; stdout only | Record row `append-chokepoint` | Any new `INSERT INTO events` site, any new module acquiring a `SHIPMENT_SEQ` stub, or any change to `tools/checks/append-chokepoint.ts` |
| Every `path:line` citation in the record resolves to a real file and an in-bounds line; anchored ones still sit within ±2 of their symbol | `pnpm check:citations` | M | Synthesized; stdout only | Record row `citations` | Any edit that shifts lines in a cited file, or any new citation |
| Every `§N` reference in the record and in source comments resolves to a section that exists | `pnpm check:section-refs` | M | Synthesized; stdout only | Record row `section-refs` | Any new `§N` reference, or a deleted section |
| Every markdown table row matches its header's column count — no cell is silently DROPPED at render | `pnpm check:tables` | M | Synthesized; stdout only | Record row `table-shape` | Any markdown table edit, especially one quoting a value containing a pipe |

Counts: **26** gates under `--profile merge` (**17** plain + 9 skippable), **31** under `--profile release`
