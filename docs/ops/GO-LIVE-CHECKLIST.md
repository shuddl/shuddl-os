...
| Status-page server-side geo generalization | threat-model.md:37 · REQ-074 | `generalizePosition` is client-consumed; true boundary is server-side lens scoping (WP-02) — must hold before public status pages | Enforce generalization server-side before public status go-live | **Med** |
...
| CMS/cert-chain TSA receipt verification deferred | `packages/ledger/src/tsa/client.ts:9`; `der.ts` | Raw `.tsr` stored in R2 (verifiable offline forever); only structural DER parse now | WP-16 CMS signature + cert-chain verify | **Low** |
...
| Security debt | `docs/ops/threat-model.md` | Asset map, standing mitigations, TSA/tile provenance, suppression list | Still-open: TSA CMS verify (WP-16), real TSA endpoint (F1 CONFIRM), status-page server-side geo boundary |
