...
| **`provision-prod --apply` can leave orphan resources if pointed at the wrong account** — it creates first and refuses second, so a wrong `--account-id` that still passes the marketing-account heuristic can produce up to 9 unreferenced Cloudflare resources before the post-create clobber-check aborts without writing any config | Med | Repo (`tools/deploy/provision-prod.ts`) | Design read, reproducible by construction: the pre-create `planPatches` sees no resolved ids (nothing adopted in an empty account), so every resource is created, and only the post-create pass raises `would-clobber-real-id` and aborts. The abort is correct — no config is written — but the resources persist. Surfaced by the agent that built the tool, 2026-07-30 | backend | OPEN | none — provisioning is an operator command, not a release gate | when `tools/deploy/provision-prod.ts` changes, or when the pre-create pass is taught to resolve ids first |
