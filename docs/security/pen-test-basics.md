...
**2026-07-27 re-verdict (delta only).** The close-out opened one genuinely new surface — a third party owning an input to a CI gate — and closed it for the capture, with a hard assertion that no uncontrolled origin completed a request (§3.1). It also gave the deploy-configuration surface a perimeter that asserts its own requirements instead of assuming them. Three items are recorded honestly rather than closed: the production basemap is still third-party at runtime (REQ-075, a deploy hold, not a CI question, §5); the release record's binding fields are self-satisfied until a promote step derives its own context (§5); and no in-repo check can prove a resource is provisioned rather than merely plausible — only a `--state` snapshot of the account can (§4). Nothing in §2 regressed, and nothing there was re-executed in this pass — the delta claims only what its own suites prove. **Still clean for the in-repo perimeter; still not deployable, and the gate still says so** (`preflight --env prod`: 26 BLOCKs).
