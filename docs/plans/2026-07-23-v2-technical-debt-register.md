...
| 990 tender-response — best-effort, no-ops when transport unwired | REQ-201 | `packages/edi/src/build-990.ts`, `inbound.ts:532-547` | Serializer + dedupe built + byte-stable; `send990` throws under NotConfigured, so ack deferred (204 still recorded). Lands with transport. | degrades→cosmetic |
