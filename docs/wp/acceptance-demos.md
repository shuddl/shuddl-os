...
- **⚠ PREREQUISITE — there is no signup SURFACE (audit §237).** `POST /pub/signup` is a raw JSON API and **no product surface calls it**: verified exhaustively — no `<form>`, no email input and no submit handler exists in `apps/command`, `apps/driver`, `apps/portal` or any worker, and this repo contains no landing page. So *"a stranger, unassisted, from landing to first quote"* cannot be filmed: the only way to sign up today is to hand-issue HTTP requests, which is neither unassisted nor a demonstration of a product. The absence is disclosed elsewhere ONLY as a browser-**test** gap (`browser: null` in `tools/acceptance/demos.ts`), which is why it reads as a coverage limitation rather than the filming blocker it is. **Film this only after a signup surface exists** — or film it honestly as an API walkthrough and label it that way. Do not film an operator typing `curl` and narrate it as a stranger signing up (the same fault as staging photos into demo 1, or a login into demo 3). The flag/legal prerequisites already named — `PROVISIONING_ENABLED` on, ToS/CONFIRM-2 in place — still stand on top of this.
...
- **⚠ PREREQUISITE — nothing in the shipped code creates the `pairings` row this demo authenticates against (audit §1773).** The control plane has four tables and `provisionTenant` mints **three** of them in one atomic batch — the `tenants` row (an UPDATE claiming an unclaimed pool slot), the admin `users` row, the `usage_credits` row — and **stops one table short**. The only `INSERT INTO pairings` statements in `packages/`, `workers/` or `tools/` are test fixtures. MCP `/register` does not fill the gap: a client registers **against an existing active `mcp` pairing**, and `authenticateClient` refuses an absent or non-`active` row — correctly fail-closed, which is exactly why the row must already be there. No runbook documents the manual control-plane INSERT, while `docs/ops/GO-LIVE-CHECKLIST.md`'s *Per-pairing caps provisioning* row instructs an operator to set `caps` on every `kind='mcp'` pairing — an instruction that presupposes pairings exist. The bullet above says *"per-pairing caps provisioned"*, which reads as a configuration step and hides that **the row itself has no shipped provisioner**. So this demo cannot be run on a freshly-provisioned tenant. Film it only after the pairing and its REQ-105 caps exist, and **record that the provisioning was manual** — an operator hand-inserting a control-plane row off camera is the same fault as staging photos into demo 1. The gap is wider than MCP: `kind='webhook'` and `kind='edi'` share the table, and the EDI 204 seam authenticates off `pairings.secret_ref`.
...
- **⚠ PREREQUISITE — the greige world is not ours yet (audit §1773).** REQ-075's self-hosted Protomaps vectors on R2 have not landed, and the style points at a third-party endpoint; the blocker is filed in `docs/ops/GO-LIVE-CHECKLIST.md` §2 (*Self-hosted Protomaps vectors on R2*, **blocks go-live**) with the third-party stand-in recorded beside it. **The dim behaviour is ours and is asserted; the basemap under it is rented.** Film knowing the ground is a stand-in, or land the R2 self-host first. This note exists because the blocker was filed on the checklist and **nowhere in this manifest** — the same disclosure gap §237 named for demo 2, where an absence recorded only in one place reads as a different kind of problem than it is.
