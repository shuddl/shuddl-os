...
7. **Design CI**: color/contrast/font/case/radius/shadow/motion audits + 5 blessed screenshots. `--signal-deep` is tuned by the contrast test, not by eye. **Advisory (report-only) until WP-10 exits, blocking thereafter (REQ-158)** — pixel law must not stall ledger work.
